...
interface IndustrialVisitsProps {
  isAdmin?: boolean;
}

export default function IndustrialVisits({ isAdmin = false }: IndustrialVisitsProps) {
  const [photos, setPhotos] = useState<Photo[]>(() => loadPhotos());

  // Persist every change so admin uploads survive reloads.
  useEffect(() => {
    try {
    ...
    <div>
      <div className="mb-10 flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="mb-1 text-sm font-semibold uppercase tracking-widest text-blue-600">Real Industry Exposure</p>
            <h2 className="text-4xl font-extrabold text-slate-950">Industrial Visits</h2>
          </div>
        </div>
